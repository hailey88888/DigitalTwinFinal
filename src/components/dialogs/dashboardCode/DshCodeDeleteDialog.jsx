...
import { dashboardCodeClient,deleteDshboardCode } from '../../../util/restAPI/siteAdmin/dashboardCode';

const DshCodeDeleteDialog = () => {
    ...
    const deleteClose = () => {
        dispatch(dshcodeActions.getDshCodeDeleteDialog(false));
    ...
    const deleteHandle = () => {
        checkedBoxes.forEach(async (checkedID) => {
            ...
            mutate({ dashCode: checkedID, siteNum: siteNo });
            dispatch(dshcodeActions.getDshCodeDeleteDialog(false));
        });   
    }
   ...
   const { mutate } = useMutation({
    mutationFn :deleteDshboardCode,  
    onSuccess: () => {
        dashboardCodeClient.invalidateQueries({
        queryKey: ['getDshboardCodeList'],
        refetchType: 'none',
        });
    },
    });
...
export default DshCodeDeleteDialog;