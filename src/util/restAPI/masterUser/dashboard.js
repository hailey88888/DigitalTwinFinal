...
//대시보드  전체 조회 
export async function getDashboardlists({ signal,page,site_id }) {
    const URL = `${BASE_URL}/HOME/site/${site_id}/dashboard?page=${page}&size=15`
    ...
    if (!response.ok) {
        ...
        const { dashboardList, totalCount } = await response.json();
        console.log('API getList-URL : ',URL)
        console.log('API getList-totalPages',totalCount);
        console.log('API getList-content',dashboardList);
    
        const content = { dashboardList, totalCount } ;
    return content;
...
export async function createDshCode({body,sessionID}) {
    const URL = `${BASE_URL}/HOME/site/${site_id}/dashboard`
