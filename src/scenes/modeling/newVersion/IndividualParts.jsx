...
import { useRef, useState,useMemo, forwardRef,Suspense} from "react";
...
const IndividualParts = forwardRef(({ comName, individualPartsData, parts_path, individualFacNum, partsData }, ref) => {
  const dispatch = useDispatch();
  const {siteNo} = useSelector((state) => state.authReducer);

  const {numOfDataRetievals} = useSelector((state) => state.siteBuildingReducer);
...
const updateRotation = () => {
  if (curValue.current !== 0 && groupRef.current) {
    // console.log("updateRotation",TOPIC_URL);
...
export default IndividualParts;
